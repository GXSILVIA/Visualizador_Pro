...
if authentication_status:
    ...
    def obtener_rango_id(vol, modo):
        ...
        if "Código Postal" in modo:
            # Rangos Polígonos: 100, 200, 300, 400
            if v == 0: return 0
            if v <= 100: return 1
            if v <= 200: return 2
            if v <= 300: return 3
            ...
            # Rangos Coordenadas: 15, 20, 30, 40
            if v == 0: return 0
            if v <= 15: return 1
            if v <= 20: return 2
            if v <= 30: return 3
    ...
    with col_controles:
        ...
        f2 = st.columns(3)
        for i in range(6):
            col_target = f1[i] if i < 3 else f2[i-3]
            if col_target.checkbox(labels[i], value=True, key=f"f_{i}_{modo}"):
                activos.append(i)
        ...
        if (archivo_excel and btn_actualizar) or (archivo_excel and st.session_state.get('last_fn') != archivo_excel.name):
            ...
            df_proc = df_raw.rename(columns={'VOLUMEN':'VOL', 'C.P.':'CP', 'CODIGO_POSTAL':'CP', 'VOL':'VOL'})
    ...
    with col_mapa:
        if st.session_state.df_datos is not None:
            ...
            if "Código Postal" in modo:
                ...
                if gdf is not None:
                    ...
                    for _, fila in merged.iterrows():
                        ...
                        folium.GeoJson(fila['geometry'], style_function=lambda x, color=c: {
                            'fillColor': color, 'color': color, 'weight': 1.5, 'fillOpacity': 0.4
                        }).add_to(m)
                        if ver_nombres and fila["VOL"] > 0:
                            cen = fila['geometry'].centroid
                            html = f'<div style="font-size: 8pt; font-weight: bold; text-align: center; width: 120px; text-shadow: 1px 1px 2px white;">{fila.get("NOMBRE","")}<br><span style="color: #d32f2f;">({int(fila["VOL"])})</span></div>'
                ...
                for _, fila in df_gps.iterrows():
                    ...
                    folium.CircleMarker(
                        ...
                        radius=8, color=c, fill=True, fill_color=c, fill_opacity=0.8,
            ...
            st.download_button(
                label="💾 Descargar Mapa Actual (Archivo HTML)",
                data=mapa_html,
                file_name=f"mapa_amzl_{datetime.now().strftime('%H%M%S')}.html",
                mime="text/html",
