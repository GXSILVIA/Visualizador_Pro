...
if auth_status:
    ...
    with col_panel:
        ...
        if st.session_state.dict_datos:
            fecha_sel = st.select_slider("🕒 Historial:", options=list(st.session_state.dict_datos.keys()))
            ...
            ver_nombres = st.toggle("🏷️ Ver Nombres en Mapa", value=True)
    ...
    with col_mapa:
        if st.session_state.dict_datos:
            ...
            map_out = st_folium(m, width="100%", height=550, key=f"map_{fecha_sel}_{modo}")
    ...
    # --- 5. INFORME DEBAJO DEL MAPA (Ancho Completo) ---
    if st.session_state.dict_datos:
        st.markdown("---")
        df_base = df_act if (modo_analisis if 'modo_analisis' in locals() else False) else df_m
        ...
        if st.session_state.zona_seleccionada:
            ...
            if not sel.empty:
                ...
                if not df_rep.empty:
                    if len(df_rep[df_rep['%_ENCIMADO'] >= 99]) > 1:
                        st.error("🚨 DUPLICIDAD AL 100% DETECTADA: Varios responsables operando en el mismo punto.")
                    
                    rep_col1, rep_col2 = st.columns([1, 2])
                    with rep_col1:
                        st.write("**Carga de Trabajo en el Área (Volumen):**")
                        st.bar_chart(df_rep.groupby('PER')['VOL'].sum())
                        
                        btn_c1, btn_c2 = st.columns(2)
                        if btn_c1.button("🗑️ Limpiar Selección", use_container_width=True): 
                            st.session_state.zona_seleccionada = None
                            st.rerun()
                        
                        buf_r = io.BytesIO()
                        with pd.ExcelWriter(buf_r, engine='openpyxl') as writer:
                            df_rep[['NOM', 'PER', 'VOL', '%_ENCIMADO']].to_excel(writer, index=False)
                        btn_c2.download_button("💾 Exportar Excel", data=buf_r.getvalue(), file_name="traslapes_amzl.xlsx", use_container_width=True)
                    ...
                    with rep_col2:
                        ...
                        st.write("**Desglose de Responsables y Traslapes:**")
                        st.dataframe(
                            df_rep[['NOM', 'PER', 'VOL', '%_ENCIMADO']].style.applymap(est, subset=['%_ENCIMADO']).format({'%_ENCIMADO': '{:.1f}%'}), 
                            use_container_width=True, height=300
                        )
        else:
            st.info("👆 Selecciona un punto en el mapa para desplegar aquí el informe de traslapes y volumen.")

elif auth_status is False: st.error('Usuario/Contraseña incorrectos')
elif auth_status is None: st.warning('Ingrese credenciales para acceder al sistema.')
