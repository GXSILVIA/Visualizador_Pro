...
def area_interseccion(r1, r2, d):
    ...
    p1, p2 = r1**2, r2**2
    p3_v = (d**2 + r1**2 - r2**2) / (2 * d * r1)
    p4_v = (d**2 + r2**2 - r1**2) / (2 * d * r2)
    p1_a = p1 * np.arccos(np.clip(p3_v, -1, 1))
    p2_a = p2 * np.arccos(np.clip(p4_v, -1, 1))
...
if status:
    ...
    with col_p:
        ...
        if modo == "Polígonos CP":
            ...
            if edo_sel:
                ...
                if gdf is not None: 
                    b = gdf.total_bounds; bounds = [[b, b], [b, b]]
        ...
        if st.session_state.dict_datos:
            ...
            if modo == "Mapa de Calor":
                ...
                # CONTROLES PARA LOGRAR EL EFECTO DE LA IMAGEN
                rad_h = st.slider("🔥 Tamaño núcleos (Radio):", 5, 40, 15)
                blur_h = st.slider("☁️ Suavizado (Blur):", 5, 40, 15)
    ...
    with col_m:
        if st.session_state.dict_datos:
            m = folium.Map(location=[19.17, -96.13], zoom_start=12, tiles="CartoDB Voyager")
            rep = []
            if modo == "Mapa de Calor":
                dh = df_act[['LAT', 'LON']].dropna()
                dh = dh[dh['LAT'] != 0]
                if not dh.empty:
                    # GRADIENTE PERSONALIZADO PARA QUE SE VEA COMO TU IMAGEN
                    gradiente = {0.2: 'blue', 0.4: 'cyan', 0.6: 'lime', 0.8: 'yellow', 1: 'red'}
                    HeatMap(dh.values.tolist(), radius=rad_h, blur=blur_h, min_opacity=0.4, gradient=gradiente).add_to(m)
                ...
                if modo == "Polígonos CP" and gdf is not None:
                    ...
                    for _, r in gdf.iterrows():
                        ...
                        if cp in vd:
                            folium.GeoJson(r['geometry'], style_function=lambda x, v=vd[cp]: {'fillColor':clrs[obtener_rango_id(v,True)], 'color':'#000', 'weight':1, 'fillOpacity':0.4}, tooltip=f"<b>{nd[cp]}</b><br>Vol: {int(vd[cp])}").add_to(m)
                            if ver_n:
            ...
            if modo == "Mapa de Calor":
                ...
                with c2:
                    if rep:
                        ...
                        st.download_button("📊 Informe", data=b_r.getvalue(), file_name="analisis.xlsx", use_container_width=True)
