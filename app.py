...
def area_interseccion(r1, r2, d):
    ...
    # np.clip previene errores numéricos fuera del rango [-1, 1] para arccos
...
if auth_status:
    def normalizar_df(df, modo_ref):
        ...
        # Definición de rangos según etiquetas solicitadas
    ...
    with col_controles:
        ...
        # Plantilla usando openpyxl
        buffer_plt = io.BytesIO()
        with pd.ExcelWriter(buffer_plt, engine='openpyxl') as writer:
            pd.DataFrame(columns=['LAT', 'LON', 'VOL', 'RAD', 'CP', 'NOMBRE', 'RESPONSABLE']).to_excel(writer, index=False)
        st.download_button("📥 Descargar Plantilla Excel", data=buffer_plt.getvalue(), file_name="plantilla_amzl.xlsx")
        ...
        if st.session_state.dict_datos:
            ...
            if not df_m.empty:
                ...
                if st.session_state.zona_seleccionada:
                    sel_rows = df_act[df_act['COORD_KEY'] == st.session_state.zona_seleccionada]
                    if not sel_rows.empty:
                        m_row = sel_rows.iloc[0]
                        ...
                        if not df_rep.empty:
                            ...
                            with c1:
                                ...
                                # Botón para exportar informe actual
                                buffer_rep = io.BytesIO()
                                with pd.ExcelWriter(buffer_rep, engine='openpyxl') as writer:
                                    df_rep[['NOM', 'PER', 'VOL', '%_ENCIMADO']].to_excel(writer, index=False, sheet_name='Traslapes')
                                st.download_button("💾 Exportar Informe de Traslapes", data=buffer_rep.getvalue(), file_name=f"reporte_traslapes_{m_row['COORD_KEY']}.xlsx")
                            ...
                            with c2:
                                ...
                                st.dataframe(df_rep[['NOM', 'PER', 'VOL', '%_ENCIMADO']].style.applymap(est, subset=['%_ENCIMADO']).format({'%_ENCIMADO': '{:.1f}%'}), use_container_width=True, hide_index=True)
                else: st.info("👆 Selecciona un punto para ver el desglose de traslapes.")
