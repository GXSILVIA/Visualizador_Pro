...
if st.session_state.get("authentication_status"):
    ...
    with col_m:
        ...
        # --- BLOQUE FINAL SUPREME: UN SOLO MAPA + RÉPLICA EXACTA DE DASHBOARD ---
        if 'm' in locals():
            st.write("---")
            ...
            # 2. RENDERIZADO ÚNICO DEL MAPA (Aquí se muestra el mapa en la app)
            map_data_final = m.get_root().render()
            components.html(map_data_final, height=500) 
            ...
            nombre_xlsx = f"REPORTE_{datetime.now().strftime('%d_%m_%Y')}.xlsx"
            buf = io.BytesIO()
            ...
            with pd.ExcelWriter(buf, engine='xlsxwriter') as wr:
                ...
                f_hdr    = wb.add_format({'bold': True, 'bg_color': '#1F4E78', 'font_color': 'white', 'border': 1, 'align': 'center', 'valign': 'vcenter', 'font_size': 9})
                ...
                if modo == "Crecimiento" and st.session_state.historico_resumen:
                    ...
                    # Estructura de Filas Horizontal
                    ...
                    for h in st.session_state.historico_resumen:
                        ...
                        total = len(df_m) or 1
                        ws.write(4, col, h['Mes'].upper(), f_hdr)
                        ws.write(5, col, h['Prom']/100, f_perc)
                        for n_min, n_max, rb in [(0,25,7),(25,50,11),(50,75,15),(75,100,19)]:
                            ...
                            ws.write(rb, col, count/total, fmt)
                            ws.write(rb+1, col, count, f_val); ws.write(rb+2, col, int(pvol), f_val)
                        ws.write(23, col, total, f_val); ws.write(24, col, int(df_m['VOL'].mean()), f_footer)
                        col += 1

                    # Tendencia (Sparklines) - LISTA CORREGIDA PARA EVITAR [0:0]
                    lc = xlsxwriter.utility.xl_col_to_name(col-1)
                    ws.write(4, col, "TENDENCIA", f_hdr)
                    for r_sp in [5, 7, 11, 15, 19, 23, 24]:
                        ws.add_sparkline(r_sp, col, {'range': f'RESUMEN!B{r_sp+1}:{lc}{r_sp+1}', 'type': 'column', 'style': 18})

                    # --- LAS 3 GRÁFICAS (RÉPLICA DE IMAGEN) ---
                    ...
                    for n, r, c in [('Bajo',7,'#92D050'),('Media',11,'#FFFF00'),('Alto',15,'#FFC000'),('Crítico',19,'#FF0000')]:
                    ...
                    l3.add_series({'name': 'VRs', 'values': f'=RESUMEN!$B$24:${lc}$24', 'line': {'color': '#C00000'}, 'data_labels': {'value': True}})
                    g3.combine(l3); g3.set_title({'name': 'DILUCION DE VOLUMEN'}); ws.insert_chart('L28', g3, {'x_scale': 1.1})
