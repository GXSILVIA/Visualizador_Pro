#-*- coding: utf-8 -*-
# Copyright 2026 Silvia Guadalupe Garcia Espinosa - Sistema Pro AMZL v8.2 Fix Polygons
...
if st.session_state.get("authentication_status"):
    ...
    with col_p:
        ...
        # Lógica de Polígonos (Selector de Estado)
        ...
        if modo == "Polígonos CP":
            if os.path.exists('mapas'):
                ...
                if archs:
                    ...
                    idx_f = [f.replace('.geojson','').replace('_',' ') for f in archs].index(edo_sel)
                    gdf_pol = gpd.read_file(f"mapas/{archs[idx_f]}").to_crs("EPSG:4326")
                    col_cp_geo = next((c for c in ['d_cp','CP','CODIGOPOSTAL'] if c in gdf_pol.columns), gdf_pol.columns[0])
                    b = gdf_pol.total_bounds
                    b_pol = [[float(b[1]), float(b[0])], [float(b[3]), float(b[2])]]
    ...
    with col_m:
        ...
        if not hay_d: st.info("👋 Por favor, procesa un archivo.")
        else:
            ...
            if modo == "Crecimiento":
                ...
                data_p = [r for r in st.session_state.analisis_cache[hoja_act] if r['R_ID'] in acts]
                for p in data_p:
                    folium.Circle([p['LAT'], p['LON']], radius=p['RAD'], color=clrs[p['R_ID']], fill=True, fill_opacity=0.3, tooltip=f"{p['Zona']}: {p['Traslape']}%").add_to(m)
                    if ver_n: folium.Marker([p['LAT'], p['LON']], icon=folium.features.DivIcon(html=f'<div style="font-size:8pt; font-weight:bold; color:#000; text-shadow: 0 0 1px #FFF; width:100px;">{p["Zona"]}</div>')).add_to(m)
                if data_p: m.fit_bounds([[min(r['LAT'] for r in data_p), min(r['LON'] for r in data_p)], [max(r['LAT'] for r in data_p), max(r['LON'] for r in data_p)]])
            
            elif modo == "Polígonos CP" and gdf_pol is not None:
                df_pol_v = st.session_state.df_datos[st.session_state.df_datos['R_ID'].isin(acts)].set_index('CP')
                for _, r in gdf_pol.iterrows():
                    cp_geo = str(r[col_cp_geo]).zfill(5)
                    if cp_geo in df_pol_v.index:
                        vol_p = df_pol_v.loc[cp_geo, 'VOL']
                        nom_p = df_pol_v.loc[cp_geo, 'NOM']
                        folium.GeoJson(r['geometry'], style_function=lambda x, v=vol_p: {'fillColor':clrs[obtener_rango_id(v,True)], 'color':'#000', 'weight':1, 'fillOpacity':0.4}, tooltip=f"CP: {cp_geo} | {nom_p}").add_to(m)
                        if ver_n:
                            c = r['geometry'].centroid
                            folium.Marker([c.y, c.x], icon=folium.features.DivIcon(html=f'<div style="font-size:8pt; font-weight:bold; color:#000; text-align:center; width:80px;">{nom_p}</div>')).add_to(m)
                m.fit_bounds(b_pol)
                ...
                for i, p1 in enumerate(pts):
                    ...
                    rep_coords.append({"ST": "🟢 Bajo" if tr <= 25 else "🔴 Crítico", "ZONA": p1['NOM'], "VOLUMEN": int(p1['VOL']), "% TRANSLAPE REAL": f"{tr}%"})
            ...
            if m_ana:
                ...
                if modo == "Crecimiento":
                    ...
                    st.dataframe(df_ex[["ST", "Zona", "Traslape"]].rename(columns={"Traslape":"% TRANSLAPE REAL"}), use_container_width=True, hide_index=True)
                elif modo == "Coordenadas":
                    st.dataframe(pd.DataFrame(rep_coords), use_container_width=True, hide_index=True)

            c1, c2 = st.columns(2)
            c1.download_button("🗺️ Mapa HTML", data=m.get_root().render(), file_name=f"mapa_{modo.lower()}.html", use_container_width=True)
            if modo != "Polígonos CP":
                buf = io.BytesIO()
