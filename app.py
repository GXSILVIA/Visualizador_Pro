...
if st.session_state.get("authentication_status"):
    ...
    with col_m:
        ...
        # --- SECCIÓN DE DESCARGAS: RÉPLICA TOTAL DASHBOARD Y DETALLES (SIN FORMATO CONDICIONAL) ---
        ...
        with pd.ExcelWriter(buf, engine='xlsxwriter') as wr:
            ...
            f_title  = wb.add_format({'bold': True, 'font_size': 12, 'font_color': '#1F4E78'})
            ...
            f_vrs_vol = wb.add_format({'bg_color': '#2F5597', 'font_color': 'white', 'bold': True, 'align': 'center', 'border': 1, 'font_size': 9})
            ...
            if modo == "Crecimiento" and st.session_state.historico_resumen:
                # --- PESTAÑA RESUMEN (IMAGEN 1) ---
                ws_res = wb.add_worksheet("RESUMEN")
                ws_res.hide_gridlines(2)
                ws_res.set_column('A:A', 15)
                ws_res.set_column('B:B', 18)
                ws_res.set_column('C:J', 12)
                ws_res.set_column('K:K', 15)
                
                ws_res.merge_range('A1:K2', "EVOLUCIÓN MENSUAL Y SALUD", f_title)
                ws_res.write(4, 0, "MES", f_header)
                ws_res.write(4, 1, "TRASLAPE TOTAL", f_header)
                ws_res.write(4, 2, "NIVEL BAJO", f_header); ws_res.write(4, 3, "VRS/P.VOL", f_header)
                ws_res.write(4, 4, "NIVEL MEDIO", f_header); ws_res.write(4, 5, "VRS/P.VOL", f_header)
                ws_res.write(4, 6, "NIVEL ALTO", f_header); ws_res.write(4, 7, "VRS/P.VOL", f_header)
                ws_res.write(4, 8, "NIVEL CRÍTICO", f_header); ws_res.write(4, 9, "VRS/P.VOL", f_header)
                ws_res.write(4, 10, "TOTAL VRs", f_header)
                ...
                for i, h_res in enumerate(st.session_state.historico_resumen):
                    ...
                    for n_min, n_max, n_fmt in [(0, 25, f_v), (25, 50, f_a), (50, 75, f_n), (75, 100, f_r)]:
                        ...
                        sub = df_m[mask]; vrs_count = len(sub); p_vol = sub['VOL'].mean() if vrs_count > 0 else 0
                        ws_res.write(row_idx, col_off, vrs_count/total_m, n_fmt)
                        ws_res.write(row_idx, col_off + 1, f"{vrs_count} / {int(p_vol)}", f_vrs_vol)
                        col_off += 2
                    ws_res.write(row_idx, 10, total_m, wb.add_format({'align':'center', 'border':1, 'bold':True}))
                    row_idx += 1

                # Gráfica Doble Eje
                chart = wb.add_chart({'type': 'column'})
                line_obj = wb.add_chart({'type': 'line'})
                chart.add_series({'name': 'VRs Totales', 'categories': f'=RESUMEN!$A$6:$A${row_idx}', 'values': f'=RESUMEN!$K$6:$K${row_idx}', 'fill': {'color': '#DDEBF7'}, 'border': {'color': '#1F4E78'}, 'data_labels': {'value': True, 'position': 'inside_base'}})
                ...
                chart.combine(line_obj); chart.set_legend({'position': 'bottom'}); ws_res.insert_chart('A15', chart, {'x_scale': 2.2, 'y_scale': 1.4})

                # --- PESTAÑAS DE DETALLE (IMAGEN 2 - SIN FORMATO CONDICIONAL) ---
