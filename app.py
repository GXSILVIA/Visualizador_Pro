#-*- coding: utf-8 -*-
# Copyright 2026 Silvia Guadalupe Garcia Espinosa - Sistema Pro AMZL v7.2 Élite
            ...
            if modo == "Crecimiento":
                ...
                for i, (nombre, df_h) in enumerate(st.session_state.dict_hojas.items()):
                    ...
                    for k, p1 in enumerate(pts):
                        ...
                        res.append({"Mes": nombre, "ST": st_v, "Zona": p1['NOM'], "Traslape": tr, "R_ID": p1['R_ID'], "LAT": p1['LAT'], "LON": p1['LON'], "RAD": p1['RAD']})
        ...
        if not hay_datos: st.info("Carga un archivo para iniciar.")
            ...
            if modo == "Crecimiento":
                ...
                for i, p1 in enumerate(pts):
                    ...
                    rep.append({"ST": "Sano", "Zona": p1['NOM'], "Paquetes": int(p1['VOL']), "Traslape Real": f"{tr}%"})
            ...
            if m_ana and modo == "Crecimiento":
                ...
                st.markdown(f"<h1 style='text-align: center; color: #ff4b4b; font-size: 45px;'>{nh_all[st.session_state.idx_hoja]}</h1>", unsafe_allow_html=True)
                
                df_ex = pd.DataFrame(st.session_state.analisis_cache[nh_all[st.session_state.idx_hoja]])
                df_ex = df_ex[df_ex['ST'].isin(f_estatus)]
                b = len(df_ex[df_ex['Traslape'] <= 30]); m_v = len(df_ex[(df_ex['Traslape'] > 30) & (df_ex['Traslape'] < 80)]); c = len(df_ex[df_ex['Traslape'] >= 80]); tot = len(df_ex)
                ...
                <div style="display: flex; justify-content: space-around; background: #1e1e1e; padding: 15px; border-radius: 10px; border: 1px solid #444;">
                    <div style="text-align: center;"><p style="color: #bbb; margin:0;">📊 Promedio</p><h2 style="margin:0;">{round(df_ex['Traslape'].mean(), 1)}%</h2></div>
                    <div style="text-align: center;"><p style="color: #bbb; margin:0;">🚀 Aumento</p><h2 style="margin:0;">+{st.session_state.historico_resumen[st.session_state.idx_hoja]['Nuevas']}</h2></div>
            ...
            if modo == "Crecimiento":
                ...
                with pd.ExcelWriter(buf, engine='xlsxwriter') as writer:
                    ...
                    ws = writer.sheets['EJECUTIVO']
                    chart_ex = writer.book.add_chart({'type': 'column'})
                    chart_ex.add_series({'categories': '=EJECUTIVO!$A$2:$A$'+str(len(df_h)+1), 'values': '=EJECUTIVO!$B$2:$B$'+str(len(df_h)+1), 'name': 'Zonas'})
                    ws.insert_chart('F2', chart_ex)
                ...
                c_d2.download_button("📊 Exportar Excel Ejecutivo", data=buf.getvalue(), file_name="informe_crecimiento.xlsx", use_container_width=True)
