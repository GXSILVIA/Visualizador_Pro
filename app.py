#-*- coding: utf-8 -*-
# Copyright 2026 Silvia Guadalupe Garcia Espinosa - Sistema Pro AMZL v8.3 Total
...
if st.session_state.get("authentication_status"):
    ...
    with col_p:
        ...
        # --- PLANTILLAS (RESTAURADAS Y CORREGIDAS) ---
        st.subheader("📥 Plantillas")
        cols_base = {"Coordenadas":["ZONA","LATITUD","LONGITUD","RADIO","VOLUMEN"], "Polígonos CP":["ZONA","CP","VOLUMEN"], "Crecimiento":["ZONA","LATITUD","LONGITUD","RADIO","VOLUMEN"]}
        buf_p = io.BytesIO()
        ...
        if modo == "Polígonos CP":
            ...
            if archs:
                ...
                col_cp_geo = next((c for c in ['d_cp','CP','CODIGOPOSTAL'] if c in gdf_pol.columns), gdf_pol.columns)
    ...
    with col_m:
        ...
        if not hay_d: st.info("👋 Por favor, carga y procesa un archivo.")
        else:
            ...
            if modo == "Crecimiento":
                ...
                df_p_v = st.session_state.df_datos[st.session_state.df_datos['R_ID'].isin(acts)].set_index('CP')
                for _, r in gdf_pol.iterrows():
                    cp_g = str(r[col_cp_geo]).zfill(5)
                    if cp_g in df_p_v.index:
                        vol_p, nom_p = df_p_v.loc[cp_g, 'VOL'], df_p_v.loc[cp_g, 'NOM']
                        folium.GeoJson(r['geometry'], style_function=lambda x, v=vol_p: {'fillColor':clrs[obtener_rango_id(v,True)], 'color':'#000', 'weight':1, 'fillOpacity':0.4}).add_to(m)
                        if ver_n:
                ...
                for i, p1 in enumerate(pts):
                    ...
                    st_v = "🟢 Bajo" if tr_r <= 25 else "🟡 Medio" if tr_r <= 50 else "🟠 Alto" if tr_r <= 75 else "🔴 Crítico"
            ...
            if m_ana:
                ...
                if modo == "Crecimiento":
                    ...
                    st.subheader("📋 Análisis de Traslapes Operativo")
                ...
                with pd.ExcelWriter(buf, engine='xlsxwriter') as wr:
                    if modo == "Crecimiento":
                        ...
                        for ns in st.session_state.dict_hojas.keys():
                            pd.DataFrame(st.session_state.analisis_cache[ns])[["ST","Zona","Traslape"]].to_excel(wr, sheet_name=ns[:25], index=False)
